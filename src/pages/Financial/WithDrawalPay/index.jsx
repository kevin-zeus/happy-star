...
import withdrawalPayApi from '../../../api/financial/withdrawal-pay';
...
const TableBody = forwardRef((props, ref) => {
  ...
  const searchApi = async (params) => {
    const [, res] = await withdrawalPayApi.getList(params);
    if (res && res.result) {
      return {
...
const WithDrawalPay = () => {
  ...
  const editCurrentData = async ({ content_id }) => {
    const [, res] = await withdrawalPayApi.edit({ content_id });
    if (res) {
  ...
  const createRequest = async (values) => {
    const [, res] = await withdrawalPayApi.create(values);
    if (res) {
  ...
  const updateRequest = async (values) => {
    const [, res] = await withdrawalPayApi.update({
      ...currentData,
      ...values,
    });
    if (res) {
