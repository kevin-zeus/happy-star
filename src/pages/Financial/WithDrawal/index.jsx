...
import withdrawalApi from '../../../api/financial/withdrawal';
...
const TableBody = forwardRef((props, ref) => {
  ...
  const searchApi = async (params) => {
    const [, res] = await withdrawalApi.getList(params);
    if (res && res.result) {
      return {
...
const WithDrawal = () => {
  ...
  const editCurrentData = async ({ content_id }) => {
    const [, res] = await withdrawalApi.edit({ content_id });
    if (res) {
  ...
  const createRequest = async (values) => {
    const [, res] = await withdrawalApi.create(values);
    if (res) {
  ...
  const updateRequest = async (values) => {
    const [, res] = await withdrawalApi.update({
      ...currentData,
      ...values,
    });
    if (res) {
