...
import dynamicConfigApi from '../../../api/operator/dynamic-config';
...
const TableBody = forwardRef((props, ref) => {
  ...
  const searchApi = async (params) => {
    const [, res] = await dynamicConfigApi.getList(params);
    if (res && res.result) {
      return {
...
const Content = () => {
  ...
  const editCurrentData = async ({ content_id }) => {
    const [, res] = await dynamicConfigApi.edit({ content_id });
    if (res) {
  ...
  const createRequest = async (values) => {
    const [, res] = await dynamicConfigApi.create(values);
    if (res) {
  ...
  const updateRequest = async (values) => {
    const [, res] = await dynamicConfigApi.update({
      ...currentData,
      ...values,
    });
    if (res) {
