...
import taskConfigApi from '../../../api/operator/task-config';
...
const TableBody = forwardRef((props, ref) => {
  ...
  const searchApi = async (params) => {
    const [, res] = await taskConfigApi.getList(params);
    if (res && res.result) {
      return {
  ...
  return (
    <div>
      <Search
        schema={{
          ...filterSearchForm(props.schema, 'task_config_id', 'name', 'status'),
          column: 4,
        }}
        api={searchApi}
        displayType="row"
...
const Content = () => {
  ...
  const editCurrentData = async ({ content_id }) => {
    const [, res] = await taskConfigApi.edit({ content_id });
    if (res) {
  ...
  const createRequest = async (values) => {
    const [, res] = await taskConfigApi.create(values);
    if (res) {
  ...
  const updateRequest = async (values) => {
    const [, res] = await taskConfigApi.update({
      ...currentData,
      ...values,
    });
    if (res) {
